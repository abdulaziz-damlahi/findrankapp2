	...
	jQuery.fn.extend({
		...
		revolution: function(options) {
				...
				var defaults = {
					...
					swipe_treshold : 75,					// The number of pixels that the users must move their finger by before it is considered a swipe.
		...
		var nextli = container.find('>ul:first-child >li:eq('+opt.next+')'),
			...
			nextsh.find('.defaultimg').each(function() {
				...
				if (nextsh.find('.kenburnimg').length==0)
					nextsh.append('<div class="kenburnimg" style="position:absolute;z-index:1;width:100%;height:100%;top:0px;left:0px;"><img src="'+defimg.attr('src')+'" style="-webkit-touch-callout: none;-webkit-users-select: none;-khtml-users-select: none;-moz-users-select: none;-ms-users-select: none;users-select: none;position:absolute;width:'+imgobj.w+'%;height:'+imgobj.h+'%;"></div>');
				else {
